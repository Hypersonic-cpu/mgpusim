...
func (b *Benchmark) execCDNA3(
	...
	args := CDNA3KernelArgs{
		...
		Dim:                 b.Dim,
		Out:                 b.dOutData,
		HiddenBlockCountX:   (globalSize[0] + uint32(localSize[0]) - 1) / uint32(localSize[0]),
		HiddenBlockCountY:   (globalSize[1] + uint32(localSize[1]) - 1) / uint32(localSize[1]),
		HiddenBlockCountZ:   (globalSize[2] + uint32(localSize[2]) - 1) / uint32(localSize[2]),
